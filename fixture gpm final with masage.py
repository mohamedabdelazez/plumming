...
def get_gpm(fixture, water_type, flush_mechanism):
    ...
    if sfu < min_sfu:
        messagebox.showinfo("WSFU Warning", f"The lowest WSFU value according to IPC is {min_sfu}. Using minimum value.")
        return gpm_table[min_sfu]

    if sfu in gpm_table:
        gpm = gpm_table[sfu]
    else:
        max_sfu = max(gpm_table.keys())
        if sfu > max_sfu:
            return gpm_table[max_sfu]

        # Interpolate between two values
        prev_sfu = max(k for k in gpm_table.keys() if k < sfu)
        next_sfu = min(k for k in gpm_table.keys() if k > sfu)
