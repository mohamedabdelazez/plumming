...
def calculate_head_loss(flow_rate, inner_diameter, fluid_density, fluid_viscosity, pipe_length, pipe_material):
    ...
    flow_rate_m3s = flow_rate * 6.309e-5
    # Calculate pipe area in m^2
...
def calculate_friction_factor(diameter, reynolds, roughness):
    if reynolds < 2000:
        ...
        while True:
            f_old = f
            f = (-2.0 * math.log10((roughness / diameter) / 3.7 + 2.51 / (reynolds * math.sqrt(f)))) ** -2.0
            if abs(f - f_old) < 1e-6:
                break
        return f
...
def select_pipe():
    ...
    friction_loss = calculate_head_loss(flow_rate, inner_diameter, fluid_density=998, fluid_viscosity=0.001, pipe_length=100, pipe_material=material)
