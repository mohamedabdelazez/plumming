...
fixture_dropdown = ttk.Combobox(root, textvariable=fixture_type)
...
num_fixtures.grid(row=2, column=1, padx=10, pady=10)

# Heat source efficiency dropdown
heat_label = tk.Label(root, text="Select Heat Source:")
heat_label.grid(row=3, column=0, padx=10, pady=10)
heat_source = tk.StringVar()
heat_dropdown = ttk.Combobox(root, textvariable=heat_source, values=list(efficiency_factors.keys()))
heat_dropdown.grid(row=3, column=1, padx=10, pady=10)

# Inlet temperature entry
tin_label = tk.Label(root, text="Inlet Temperature (F):")
tin_label.grid(row=4, column=0, padx=10, pady=10)
tin_entry = tk.Entry(root)
tin_entry.grid(row=4, column=1, padx=10, pady=10)

# Outlet temperature entry
tout_label = tk.Label(root, text="Outlet Temperature (F):")
tout_label.grid(row=5, column=0, padx=10, pady=10)
tout_entry = tk.Entry(root)
tout_entry.grid(row=5, column=1, padx=10, pady=10)
...
heater_capacity_label.grid(row=8, columnspan=2, padx=10, pady=10)
...
def update_fixtures(event):
    app = app_type.get()
    if app in aspe_data:
        fixture_dropdown['values'] = list(aspe_data[app].keys())
...
def calculate_heater_capacity():
    ...
    # Add to history
    #history_listbox.insert(tk.END, f"Tank Heater Capacity: {tank_heater_capacity_btu:.2f} BTU/hr | {tank_heater_capacity_kw:.2f} kW\n"
                               #    f"Instantaneous Heater Capacity: {instantaneous_heater_capacity_btu:.2f} BTU/hr | {instantaneous_heater_capacity_kw:.2f} kW\n"
                                #   f"Storage Capacity: {storage_capacity:.2f} GPH")

# Calculate GPH button
calc_button = tk.Button(root, text="Calculate GPH", command=calculate)
calc_button.grid(row=6, column=0, padx=10, pady=10)
...
heater_capacity_button.grid(row=6, column=1, padx=10, pady=10)
